        ...
        try {
          ...
          if (isVerified) {
            ...
            if (formattedPhoneNumber) {
              const phoneAsNumber = Number(formattedPhoneNumber); // Convert phone number to a number
              dispatch(setPhoneNumber(phoneAsNumber));
              console.log('Formatted Phone Number', typeof phoneAsNumber); // Ensure it's now a number
            } else {
              return;
            }
          } else {
  ...
  return (
    <ParentView
      ...
      <BottomButton>
        <Button
          bgcolor={
            otp.length === 6 && isActive ? colors.PRIMARY : colors.LIGHT_GRAY
          } // Button is only enabled if the OTP is valid and the timer is active
          text="Continue"
          textColor={
            otp.length === 6 && isActive ? colors.BLACK : colors.LIGHT_GRAY200
          }
